...
tf.summary.FileWriter("./graphlogs", detection_graph)
exit(0)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if scores[0] > 0.8:
            (left, right, top, bottom) = (boxes[0][1], boxes[0][3],
                                      boxes[0][0], boxes[0][2])
            x = (left + right) / 2
            y = (top + bottom) / 2
            (x, y) = (x * screenWidth, y * screenHeight)
            if default_area is None:
                default_area = (right - left) * (bottom - top)
            else:
                curr_area = (right - left) * (bottom - top)
                if curr_area <= default_area * 0.8:
                    if not clicked:
